 ...
 * - GLFW: biblioteca que crea la ventana y maneja teclado/ratón. OpenGL solo
 * dibuja;
 * no crea ventanas, por eso usamos GLFW.
 * - GPU: tarjeta gráfica. Los "shaders" son pequeños programas que se ejecutan
 * ahí.
 * - Shader: código que dice cómo transformar vértices (vertex shader) y qué
 * color
 * dar a cada píxel (fragment shader). Se escriben en un lenguaje parecido a C
 * (GLSL).
 * - Vértice: un punto en 3D (x, y, z). Un triángulo tiene 3 vértices.
 * - VAO/VBO: formas de guardar en la GPU los datos de los vértices (posiciones,
 * etc.).
 */
public class App {

    // Referencia a nuestra ventana. GLFW devuelve un "handle" (número largo) para
    // identificarla.
    private long window;
    ...
    // VAO = Vertex Array Object. Es como un "dossier" que guarda toda la
    // configuración de
    // un objeto (qué buffer usar, cómo leer los datos). Así luego solo decimos "usa
    // este VAO"
    // y OpenGL ya sabe cómo dibujar el triángulo.
    private int vao;

    // VBO = Vertex Buffer Object. Es un bloque de memoria en la GPU donde guardamos
    // las posiciones de los 3 vértices del triángulo (las coordenadas x, y, z de
    // cada uno).
    private int vbo;
    ...
    /**
     * Punto de entrada: inicializar todo, ejecutar el bucle de dibujo, y al salir
     * limpiar.
     */
    public void run() {
    ...
    private void init() {
        ...
        // Queremos OpenGL 3.3 (versión 3, revisión 3). Nuestros shaders usan "#version
        // 330".
        // En Mac es obligatorio pedir una versión concreta.
        GLFW.glfwWindowHint(GLFW.GLFW_CONTEXT_VERSION_MAJOR, 3);
        GLFW.glfwWindowHint(GLFW.GLFW_CONTEXT_VERSION_MINOR, 3);
        // "Core profile": solo funciones modernas de OpenGL (sin compatibilidad con
        // código muy antiguo).
        GLFW.glfwWindowHint(GLFW.GLFW_OPENGL_PROFILE, GLFW.GLFW_OPENGL_CORE_PROFILE);
        // En macOS hay que activar "forward compatibility" para usar el Core profile.
        GLFW.glfwWindowHint(GLFW.GLFW_OPENGL_FORWARD_COMPAT, GLFW.GLFW_TRUE);

        // Crear la ventana: ancho, alto, título, monitor (0 = ventana normal), ventana
        // para compartir (0 = ninguna).
        ...
        // Así evitamos "tearing" (imagen partida) y no dibujamos más frames de los que
    ...
    // 1. VERTEX SHADER: se ejecuta por cada VÉRTICE (en nuestro triángulo, 3
    // veces).
    // Su trabajo es decir "dónde" va ese vértice en la pantalla (posición en
    // 2D/3D).
    // 2. FRAGMENT SHADER: se ejecuta por cada PÍXEL que ocupa el triángulo (cientos
    // o miles).
    // Su trabajo es decir "de qué color" se pinta ese píxel.
    // Sin el vertex shader, la GPU no sabría dónde colocar los puntos. Sin el
    // fragment shader,
    // no sabría qué color dar a cada píxel. Por eso hacen falta los dos.
    //
    // ¿EN QUÉ LENGUAJE ESTÁN ESCRITOS?
    // ---------------------------------
    // En GLSL (OpenGL Shading Language). Es un lenguaje parecido a C, creado para
    // programar
    // la GPU. No es Java: son cadenas de texto (vertexSrc, fragmentSrc) que
    // enviamos a
    // OpenGL; la GPU las compila y las ejecuta. Por eso en el código Java son
    // simplemente
    // String con el código fuente en GLSL.
    //
    private void crearShaders() {
        // --- vertexSrc: código GLSL del VERTEX SHADER ---
        // Recibe la posición de cada vértice (aPos) y la asigna a gl_Position (salida
        // estándar).
        // "layout (location = 0) in vec3 aPos" = entrada en el canal 0, 3 floats (x, y,
        // z).
        ...
        // "fragColor" = color de salida del píxel. vec4(R, G, B, A) = rojo, verde,
        // azul, alpha.
        // --- fragmentSrc: código GLSL del FRAGMENT SHADER ---
        // Se ejecuta por cada píxel del triángulo. "fragColor" es la salida (color del
        // píxel).
        // vec4(R, G, B, A) = rojo, verde, azul, transparencia. (0.2, 0.8, 0.4, 1.0) =
        // verde.
        ...
        // Comprobar que el enlace fue correcto (si hay error, los shaders no son
        // compatibles, etc.).
        ...
        // Los shaders ya están copiados dentro del programa; podemos borrarlos para
        // liberar.
        GL20.glDeleteShader(vertexShader);
        GL20.glDeleteShader(fragmentShader);
    }

    // Si el shader no compiló bien (error de sintaxis en GLSL, etc.), lanzamos
    // excepción con el mensaje.
    ...
    // En OpenGL las coordenadas suelen ir de -1 a 1: (-1,-1) = esquina inferior
    // izquierda,
    ...
    private void crearTriangulo() {
        ...
        // Más adelante, con solo hacer "bind" de este VAO, OpenGL sabe qué triángulo
        // dibujar.
        ...
        // Enviar los datos al VBO. GL_STATIC_DRAW = "no vamos a cambiar estos datos"
        // (optimización).
        ...
        // Desenlazar VBO y VAO. El VAO ya guardó la configuración; no hace falta
        // dejarlos activos.
    ...
    // 4. Intercambiamos buffers (doble buffer: dibujamos en uno y mostramos el
    // otro).
    ...
    private void loop() {
        while (!GLFW.glfwWindowShouldClose(window)) {
            ...
            GL30.glBindVertexArray(vao);
            // Orden de dibujo: modo GL_TRIANGLES, empezar en el vértice 0, dibujar 3
            // vértices.
            GL11.glDrawArrays(GL11.GL_TRIANGLES, 0, 3);

            // Doble buffer: hemos dibujado en un buffer "oculto"; ahora lo mostramos en
            // pantalla
            // e intercambiamos. Así el usuario no ve líneas a medias.
            GLFW.glfwSwapBuffers(window);
            // Procesar eventos del sistema (si el usuario cerró la ventana, pulsó una
            // tecla, etc.).
            GLFW.glfwPollEvents();
        }
    }
